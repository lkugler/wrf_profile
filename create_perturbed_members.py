...
def create_perturbed_profiles(n_ens, f_orig_profile_LMU='./raso.raso', outdir='./'):
    ...
    for iens in range(1, n_ens+1):
        ...
        rand_coarse2=np.random.normal(mu,sigma,nz_step)
        rand_coarse4=np.random.normal(mu,sigma,nz_step)
        mu=0.; sigma=.02 #until e24
        mu=0.; sigma=.005
        mu=0.; sigma=.001
        rand_coarse3=np.random.normal(mu,sigma,nz_step)
        z_coarse=np.linspace(np.amin(z),np.amax(z),nz_step)
        rand =np.copy(z)*0.
        #steps
        for kk in np.arange(0,nz):
            #print(kk, rand_coarse[kk/step])
            rand[kk]=np.mean(rand_coarse[int(kk/step)])
        ...
        for ii in range(ws.shape[0]):
            u[ii]=ws[ii]*np.cos(-2.*np.pi*wd[ii]/360.)
            v[ii]=ws[ii]*np.sin(-2.*np.pi*wd[ii]/360.)
        ...
        f_out = outdir + 'raso.fc.'+str(iens).zfill(3)
